    ...
    with {:ok, _} = ok_r <- Chats.update_verification(v, %{source: :join_request}),
      ...
      async_run(fn -> 
        case derestrict_chat_member_with_retry(v.chat_id, v.target_user_id) do
          {:ok, _} -> 
            Logger.info("Successfully derestricted user #{v.target_user_id} in chat #{v.chat_id} after source conversion")
          {:error, reason} -> 
            Logger.error("Failed to derestrict user #{v.target_user_id} in chat #{v.chat_id} after source conversion: #{inspect(reason)}")
