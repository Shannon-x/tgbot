defmodule PolicrMiniBot.CallAnswerChain do
  ...
  alias PolicrMiniBot.{Disposable, Worker, JoinReuquestHosting, WelcomeMessageSender}
  alias Telegex.Type.User, as: TgUser
  alias Telegex.Type.CallbackQuery

  import PolicrMiniBot.VerificationHelper
  import PolicrMiniBot.Helper, only: [
    derestrict_chat_member_with_retry: 2,
    apply_default_permissions: 2,
    async_run: 1
  ]
  ...
  defp pass_user(%{source: :joined} = v) do
    async_run(fn -> 
      case derestrict_chat_member_with_retry(v.chat_id, v.target_user_id) do
        {:ok, _} -> 
          Logger.info("Successfully derestricted user #{v.target_user_id} in chat #{v.chat_id}")
          # 发送欢迎消息
          send_welcome_message_to_user(v)
        {:error, reason} -> 
          Logger.error("Failed to derestrict user #{v.target_user_id} in chat #{v.chat_id}: #{inspect(reason)}")
          # 尝试获取群组默认权限并应用
          apply_default_permissions(v.chat_id, v.target_user_id)
      end
    end)
  ...
  # 发送欢迎消息给通过验证的用户
  defp send_welcome_message_to_user(verification) do
    # 获取用户信息
    case Telegex.get_chat_member(verification.chat_id, verification.target_user_id) do
      {:ok, %{user: user}} ->
        WelcomeMessageSender.send_welcome_message(verification.chat_id, user)
      _ ->
        Logger.warning("Failed to get user info for welcome message, user_id: #{verification.target_user_id}")
    end
  end
end
