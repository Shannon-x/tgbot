defmodule PolicrMiniBot.Helper do
  ...
  如果失败会自动重试最多3次，每次间隔1秒。
  """
  def derestrict_chat_member_with_retry(chat_id, user_id, retry_count \\ 3) do
    case derestrict_chat_member(chat_id, user_id) do
      {:ok, _} = ok_result -> 
        ok_result
      {:error, %Telegex.RequestError{reason: :timeout}} when retry_count > 0 ->
        Logger.warning("Derestrict timeout, retrying... (remaining: #{retry_count - 1})")
        :timer.sleep(1000)
        derestrict_chat_member_with_retry(chat_id, user_id, retry_count - 1)
      {:error, %Telegex.Error{description: "Bad Request: not enough rights to restrict/unrestrict chat member"}} ->
        Logger.error("Bot lacks permission to restrict/unrestrict members in chat #{chat_id}")
        {:error, :no_permission}
      {:error, %Telegex.Error{description: "Bad Request: can't remove chat owner"}} ->
        # 群主不需要解除限制
        {:ok, true}
      error ->
        error
  ...
  当无法解除用户限制时，尝试获取群组的默认权限设置并应用。
  """
  def apply_default_permissions(chat_id, user_id) do
    with {:ok, chat} <- Telegex.get_chat(chat_id),
         %{permissions: permissions} when not is_nil(permissions) <- chat do
      # 应用群组的默认权限
      case Telegex.restrict_chat_member(chat_id, user_id, permissions) do
        {:ok, _} = ok_result -> 
          Logger.info("Applied default group permissions to user #{user_id} in chat #{chat_id}")
          ok_result
        error -> 
          Logger.error("Failed to apply default permissions: #{inspect(error)}")
          error
      end
    else
      _ -> 
        Logger.error("Failed to get chat info or permissions for chat #{chat_id}")
        {:error, :failed_to_get_chat_info}
    end
  end
